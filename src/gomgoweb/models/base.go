...
func GetDB() (*mgo.Database, *mgo.Session) {
	session, err := mgo.Dial(gomgoweb.DATABASE_DSN)
	if err != nil {
		panic(err.Error())
	}
	database := session.DB(gomgoweb.DATABASE)
...
func GetBandsByGenre(genreId bson.ObjectId) []MyDoc {
	...
	col2 := database.C(gomgoweb.GENRE_COL)
	var doc MyDoc
	var docs []MyDoc
...
func (this *MyDoc) GetLocation() string {
	database, session := GetDB()
	defer session.Close()
	collection := database.C(gomgoweb.LOCATION_COL)
